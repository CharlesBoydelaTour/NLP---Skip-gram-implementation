...
class SkipGram:
	...
	def trainWord(self, wordId, contextId, negativeIds):
		...
		h = self.l1[wordId,:]
		h = h.reshape((len(h),1))
		y_pred = expit(np.dot(self.l2.T, h))
		#loss without netgativesIds
		loss = -np.log(y_pred[contextId])
		#loss with netgativesIds
		for i in range(len(negativeIds)):
			loss -= np.log(-y_pred[negativeIds[i]])
		#derivates
		dl2 = np.dot(y_pred - context, h.T)
		dl1 = np.dot(self.l2, y_pred - context)
		dl1 = dl1.T.reshape(dl1.shape[0])
		#update
